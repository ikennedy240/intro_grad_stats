...
Calculate a point estimate of the population mean for household income, `hh_inc` in the `acs_df` data. Describe, in a sentence or two, the population of interest.

2. Calculate a confidence interval around our estimate
Use the `ttest` function to calculate a 95% confidence interval around the mean for `hh_inc`. Interpret the confidence interval in a sentence or two.

3. Perform a t-test to compare two means
Pick two states. Optionally plot the distribution of household incomes in those states. Compare the household income in those two states using a difference of means test and report your results. Interpret 1. the obvserveed t value, 2. the degress of freedom, 3. the pvalue, 4.the alternative hypotheses, 5. the confidence interval, and 6. the sample estimates.
