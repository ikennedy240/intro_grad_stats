...
### Trying it out on the Cook Data - Will an interaction work?
acs_21 <- acs_21 %>% mutate(cook_county = if_else(str_detect(NAME, 'Cook County'),0,1), group_col = if_else(str_detect(NAME, 'Cook County'),'Cook County','Not Cook County'))
level_int <- lm(rent ~ white_prop + cook_county + white_prop*cook_county, data = acs_21)
log_int <- lm(log(rent) ~ white_prop + cook_county + white_prop*cook_county, data = acs_21)

huxreg(level_level,log_level,log_poly,level_int, level_int2,log_int)


int_effect <- ggeffect(level_int, term = c('white_prop','cook_county[0,1]'))

int_effect %>% as_tibble() %>% mutate(group_col = if_else(group == '0', 'Cook County','Not Cook County')) %>%
ggplot(aes(x, predicted, colour = group_col, fill = group_col))+
  geom_line()+
  geom_ribbon(aes(ymin = conf.low, ymax = conf.high),alpha = 0.2, linewidth = 0)+
  geom_point(aes(white_prop, rent, colour = group_col), data = acs_21, alpha = 0.2)+
  theme_classic()

log_int_effect <- ggeffect(log_int, term = c('white_prop','cook_county[0,1]'))

log_int_effect %>% as_tibble() %>% mutate(group_col = if_else(group == '0', 'Cook County','Not Cook County')) %>%
  ggplot(aes(x, predicted, colour = group_col, fill = group_col))+
  geom_line()+
  geom_ribbon(aes(ymin = conf.low, ymax = conf.high),alpha = 0.2, linewidth = 0)+
  geom_point(aes(white_prop, log(rent), colour = group_col), data = acs_21, alpha = 0.2)+
  theme_classic()
